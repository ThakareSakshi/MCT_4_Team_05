...
//filter data according assured category
let assuredInput = document.querySelector(".flipcart-assured-category input");
assuredInput.addEventListener("change", (e) => {
  product_container.innerHTML = "";
  if (e.target.checked) {
    let filterArr = product.filter((el) => {
      return el.flipkartAssured == true;
    });
    addProducts(filterArr);
  } else {
    addProducts(product);
